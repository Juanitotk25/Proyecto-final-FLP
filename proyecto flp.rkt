...
(define raw-scan&parse
  (sllgen:make-string-parser scanner-spec-simple-interpreter grammar-simple-interpreter))

;El Analizador Léxico (Scanner)

(define raw-just-scan
  (sllgen:make-string-scanner scanner-spec-simple-interpreter grammar-simple-interpreter))

;; Wrappers que aplican preprocesado antes de llamar al parser/scanner
;; Expose raw parsers generated by sllgen
(define scan&parse raw-scan&parse)
(define just-scan raw-just-scan)

;El Interpretador (FrontEnd + Evaluación + señal para lectura )

;; Interpretador REPL que **ignora** el valor de retorno del programa.
;; De esta forma, después de ejecutar una sentencia `print` no se muestra
;; el marcador `'ok'` que antes devolvía la función `main`.
(define interpretador
  (sllgen:make-rep-loop  "--> "
    (lambda (pgm)
      ;; Evaluamos el programa y descartamos su valor de retorno.
      (eval-program pgm)
      (void))
...
(provide scan&parse just-scan interpretador)
